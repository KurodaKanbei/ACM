...
int n, m, S, T, totFlow, totCost;
int dis[N], slack[N], visit[N];
/* vertices indexed from 1 to T */
int modlable() {
	int delta = inf;
	for(int i = 1; i <= T; i++) {
	...
	for(int i = 1; i <= T; i++) if (visit[i]) dis[i] += delta;
	return 0;
}
...
int dfs(int x, int flow) {
	...
	visit[x] = 1;
	int left = flow;
	for(int i = e.last[x]; ~i; i = e.succ[i]) if (e.cap[i] > 0 && !visit[e.other[i]]) {
		int y = e.other[i];
